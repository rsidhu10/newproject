define([
    'text!templates/desktop/billing/billgroup_dd_model.html',
    'text!templates/desktop/billing/billgroup_dd_collection.html',
    'collections/billgroups',
    'text!templates/desktop/billing/billgroup_dd_selected.html',
    'backbone'
], function (aModelTemplate, aCollectionTemplate, aCollection, aModelSelectedTemplate) {

    var BillgroupModelView = Backbone.View.extend({
        tagName:'li',
    ...
    var BillgroupCollectionView = Backbone.View.extend({
        //tagName: 'tr',
        ...
        initialize:function () {
            ...
            this.collection.url = "/billgroup/habitations/" + this.options.billgroup_id;
            ...
            $('#create_bill_wizard_billgroup_row').html(this.template());
            $('#create_bill_wizard_billgroup_row').removeClass("hidden");
            $("#create_bill_wizard_billgroup_row :button").button('loading');
            ...
            this.collection.fetch({
                ...
                success:function (model, response) {
                    ...
                    $("#create_bill_wizard_billgroup_row :button").button('reset');
                    //console.log(model);
                }
            });
        },
...
//            console.log("SubdivisionCollectionView: addOne");
            var view = new BillgroupModelView({model:aModel});

            //console.log(view.render().el);
            $('#create_bill_wizard_billgroup_dd').last().append(view.render().el);
        },
...
//            console.log("checkSelections, items in collection: " + this.collection.length);
            ...
            //console.log(aModel);
            //this.collection.each(this.addOne);
            this.collection.each(function (aBillgroup) {
                //console.log(aBillgroup);
                if (aBillgroup.has("selected")) {
                    $('#create_bill_wizard_billgroup_row').empty();
                    //$('#create_bill_wizard_subdivision_row').html("<td>Division: </td><td>" + aBillgroup.get("name") + " <span class='label label-success'>Selected</span></td>");
                    newTemplate = _.template(aModelSelectedTemplate);
                    $('#create_bill_wizard_billgroup_row').html(newTemplate(aBillgroup.toJSON()));
                    $('#create_bill_wizard_billgroup_row').addClass('success');
                    $("#create_bill_wizard_billgroup_row :button").bind('click', athis.CreateJobClicked);
...
//                    });
        ...
        CreateJobClicked:function () {
            this.collection.each(function (aModel) {
                if (aModel.has("selected")) {
                    console.log('Create Billing Job for billgroup: ' + aModel.get('name'));
    ...
    return BillgroupCollectionView;
});
